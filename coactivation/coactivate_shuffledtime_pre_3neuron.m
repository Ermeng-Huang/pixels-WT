...
if ~isempty(postspk)
    ...
    suid_sess(~avail,:)=[];
    if ispc
        save(fullfile('F:','neupix','STP','coactivate','0722-3neuron',sprintf('%s_stp_%s_%d_%d.mat',...
...
%             end
        ...
        for i=1:10           
            x(1,i)=nnz((temp(i,:)==1&histpost(i0))|(temp(i,:)==2&histpost(i0))|(temp(i,:)==4&histpost(i0)))...
                /nnz(temp(i,:)==1|temp(i,:)==2|temp(i,:)==4);   %0ne neuron         
            x(2,i)=nnz((temp(i,:)==3&histpost(i0))|(temp(i,:)==5&histpost(i0))|(temp(i,:)==6&histpost(i0)))...
                /nnz(temp(i,:)==3|temp(i,:)==5|temp(i,:)==6);   %0ne neuron
            x(3,i)=nnz(temp(i,:)==7&histpost(i0))/nnz(temp(i,:)==7);            
        end
        out.(Isshuffle)=x;
        out_avail=true;        
