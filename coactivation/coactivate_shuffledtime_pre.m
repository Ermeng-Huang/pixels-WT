...
if ispc
    load(fullfile('D:','code','pixels-master','jpsth','+bz','+hist','sig.mat'),'sig'); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% path
else
...
for i=reshape(idces,1,[]) 
    fprintf('%d of %d\n',find(idces==i),dim);
    if nnz(sess_suids(:,2)==sig.suid(i,2))<=1 
        continue
    end
    if sidx>1 && nnz(suid(:,3)==sig.suid(i,2))>0         
         continue
    end
    stats.sess=sig.sess(i);
    stats.sess_suids=sig.suid(i,2);
    reg=bz.hist.tag_hist_reg(stats,'type','neupix');
...
if ~isempty(postspk)
    ...
    suid_sess(~avail,:)=[];
    if ispc
        save(fullfile('F:','neupix','STP','coactivate','0719',sprintf('%s_stp_%s_%d_%d.mat',...
...
for Isshuffle=shuffle
    if strcmp(Isshuffle,'raw')        
        ...
        temp=zeros(10,0);
        i0=reshape((0:length(tspre1)-1)*(tmax-tmin)/bin_size+(11:(tmax-tmin)/bin_size)',1,[]);
        for i=i0
%             if histpost(i)==0
%                 continue
%             end
            temp(:,end+1)=[histpre1(i-10:i-1)',histpre2(i-10:i-1)']*bitmask;
        end      
        for i=1:10           
            x(1,i)=nnz((temp(i,:)==1&histpost(i0))|(temp(i,:)==2&histpost(i0)))/nnz(temp(i,:)==1|temp(i,:)==2);            
            x(2,i)=nnz(temp(i,:)==3&histpost(i0))/nnz(temp(i,:)==3);
        end
        out.(Isshuffle)=x;
        out_avail=true;
    else
