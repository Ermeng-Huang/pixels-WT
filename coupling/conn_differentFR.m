...
for i=1:size(r,1)
 ...
 hold on
 for i=1:size(m,1)
     errorbar(1:5,m(i,:),s(i,:),'.-','CapSize',12,'Color',c{i},'Linewidth',1)
 end
...
box off
exportgraphics(gcf,'Conn_ratio_FR_0106.pdf','ContentType','vector');
...
boxplot([FR_nonsel_all;FR_inactive;FR_active],[ones(size(FR_nonsel_all,1),1)*1;ones(size(FR_inactive,1),1)*2;ones(size(FR_active,1),1)*3],'PlotStyle','traditional')   
