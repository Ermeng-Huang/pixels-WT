...
function Ordinal_correct_error(type,opt)
arguments
    type (1,:) char {mustBeMember(type,{'Ordinal(From-3Sample)','Ordinal(From-1Test)','Ordinal(Combined)'})} = 'Ordinal(Combined)'
...
load('F:\neupix\per_sec\duration_wave_merged.mat');
dura_sel=any(out.wrsp(:,1:17)<0.05,2);

meta_6=ephys.util.load_meta('delay',6);
if strcmp(type,'Ordinal(From-3Sample)')
    load('F:\neupix\per_sec\Ordinal_MI_From-3Sample_WelltrainedTrials_Corr.mat');
    ordi_sel(arrayfun(@(x,y)find(meta_6.sess==x&meta_6.allcid==y),out_.su_meta(:,1),out_.su_meta(:,3)),1)=any(out_.anovanp(:,1:7)<0.05,2);
elseif strcmp(type,'Ordinal(From-1Test)')
    load('F:\neupix\per_sec\Ordinal_MI_From-1Test_WelltrainedTrials_Corr.mat');
    ordi_sel(arrayfun(@(x,y)find(meta_6.sess==x&meta_6.allcid==y),out_.su_meta(:,1),out_.su_meta(:,3)),1)=any(out_.anovanp(:,1:11)<0.05,2);  
elseif strcmp(type,'Ordinal(Combined)')
    load('F:\neupix\per_sec\Ordinal_MI_Combined_WelltrainedTrials_Corr.mat');
    ordi_sel(arrayfun(@(x,y)find(meta_6.sess==x&meta_6.allcid==y),out_.su_meta(:,1),out_.su_meta(:,3)),1)=any(out_.anovanp(:,1:17)<0.05,2);  
end
...
types=[1,2;3,4]; % duration 3s vs 6s

for onetype=types'    
    if opt.ctx_only
        typesel=find(ordi_sel' & ismember(meta_6.mem_type,onetype) & strcmp(meta.reg_tree(2,:),'CTX') & ~strcmp(meta.reg_tree(5,:),''));
    else
        typesel=find(ordi_sel' & ismember(meta_6.mem_type,onetype));
    end
    if opt.single_bin
...
%         stats.(sprintf('delay%ds_type%d_pertrial',delay,(onetype(1)-1)/2+1))=cell(0,4);
        for ii=typesel
            fpath=fullfile(homedir,meta_6.allpath{ii},'FR_All_1000.hdf5');
            ...
            for t=1:2 %1st vs other    
                if t==1
                    trail1st=cat(1,0,diff(trials(:,8))==3);
                else
                    trail1st=1-cat(1,0,diff(trials(:,8))==3);
                end
                cs1=mean(fr(trail1st & trials(:,9)~=0 & trials(:,10)~=0 &trials(:,5)==4 & trials(:,8)==delay,suid==meta_6.allcid(ii),bins),3);
                cs2=mean(fr(trail1st & trials(:,9)~=0 & trials(:,10)~=0 &trials(:,5)==8 & trials(:,8)==delay,suid==meta_6.allcid(ii),bins),3);
                es1=mean(fr(trail1st & trials(:,10)==0 &trials(:,5)==4 & trials(:,8)==delay,suid==meta_6.allcid(ii),bins),3);
                es2=mean(fr(trail1st & trials(:,10)==0 &trials(:,5)==8 & trials(:,8)==delay,suid==meta_6.allcid(ii),bins),3);
                ...
                stats.(sprintf('delay%ds_type%d',delay,(onetype(1)-1)/2+1+2*(t-1)))(idx,:)=[mean(cs1),mean(cs2),mean(es1),mean(es2)];
                stats.(sprintf('delay%ds_type%d_pertrial',delay,(onetype(1)-1)/2+1+2*(t-1)))(idx,:)={cs1,cs2,es1,es2};
            end
            idx=idx+1;
...
if opt.plot_per_su
    ...
    for t=1:2
        subplot(2,3,1+3*t-3);
        hold on;
        ph=histogram([stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,1);stats.(sprintf('delay%ds_type%d',delay,2*t))(:,2)],-2:0.1:2,'Normalization','probability','FaceColor','r','FaceAlpha',0.4);
        nph=histogram([stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,2);stats.(sprintf('delay%ds_type%d',delay,2*t))(:,1)],-2:0.1:2,'Normalization','probability','FaceColor','k','FaceAlpha',0.4);
        xline(nanmean([stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,1);stats.(sprintf('delay%ds_type%d',delay,2*t))(:,2)]),'--r','LineWidth',1);
        xline(nanmean([stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,2);stats.(sprintf('delay%ds_type%d',delay,2*t))(:,1)]),'--k','LineWidth',1)
        xlim([-1.5,1.5]);
        ...
        subplot(2,3,2+3*t-3);
        hold on;
        ph=histogram([stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,3);stats.(sprintf('delay%ds_type%d',delay,2*t))(:,4)],-2:0.1:2,'Normalization','probability','FaceColor','r','FaceAlpha',0.4);
        nph=histogram([stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,4);stats.(sprintf('delay%ds_type%d',delay,2*t))(:,3)],-2:0.1:2,'Normalization','probability','FaceColor','k','FaceAlpha',0.4);
        xline(nanmean([stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,3);stats.(sprintf('delay%ds_type%d',delay,2*t))(:,4)]),'--r','LineWidth',1);
        xline(nanmean([stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,4);stats.(sprintf('delay%ds_type%d',delay,2*t))(:,3)]),'--k','LineWidth',1)        
        xlim([-1.5,1.5]);
        title('Error trials');
        ...
        %auc
        onecolumn=size(stats.(sprintf('delay%ds_type%d',delay,2*t-1)),1)+size(stats.(sprintf('delay%ds_type2',delay)),1);
        [xc,yc,~,aucc]=perfcurve((1:2*onecolumn)>onecolumn,[stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,1)...
            ,;stats.(sprintf('delay%ds_type%d',delay,2*t))(:,2);stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,2)...
            ,;stats.(sprintf('delay%ds_type%d',delay,2*t))(:,1)],0);
        [xe,ye,~,auce]=perfcurve((1:2*onecolumn)>onecolumn,[stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,3)...
            ,;stats.(sprintf('delay%ds_type%d',delay,2*t))(:,4);stats.(sprintf('delay%ds_type%d',delay,2*t-1))(:,4)...
            ,;stats.(sprintf('delay%ds_type%d',delay,2*t))(:,3)],0);
        subplot(2,3,3+3*t-3);
        hold on;
