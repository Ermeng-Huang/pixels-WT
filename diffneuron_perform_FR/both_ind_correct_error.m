...
arguments
    type (1,:) char {mustBeMember(type,{'both','3s','6s'})} = '6s'
    opt.single_bin (1,1) logical = true
    opt.bin (1,:) char {mustBeMember( opt.bin,{'first1','late1','sample','test','ITI'})} = 'late1'
...
for onetype=types'
    if strcmp(type,'both')
        if strcmp(opt.bin,'sample')
            if opt.ctx_only
                typesel=find(meta_3.wrs_p(4,:)<0.05 & ismember(2-sign(meta_3.selec(4,:)),onetype) ...
                    & meta_6.wrs_p(4,:)<0.05 & ismember(2-sign(meta_6.selec(4,:)),onetype) & strcmp(meta.reg_tree(2,:),'CTX') & ~strcmp(meta.reg_tree(5,:),''));                
            else
                typesel=find(meta_3.wrs_p(4,:)<0.05 & ismember(2-sign(meta_3.selec(4,:)),onetype) & meta_6.wrs_p(4,:)<0.05 & ismember(2-sign(meta_6.selec(4,:)),onetype));                
            end
        elseif strcmp(opt.bin,'test')
            if opt.ctx_only
                typesel=find(meta_3.wrs_p(8,:)<0.05 & ismember(2-sign(meta_3.selec(8,:)),onetype) ...
                    & meta_6.wrs_p(11,:)<0.05 & ismember(2-sign(meta_6.selec(11,:)),onetype) & strcmp(meta.reg_tree(2,:),'CTX') & ~strcmp(meta.reg_tree(5,:),''));                
            else
                typesel=find(meta_3.wrs_p(8,:)<0.05 & ismember(2-sign(meta_3.selec(8,:)),onetype) & meta_6.wrs_p(11,:)<0.05 & ismember(2-sign(meta_6.selec(11,:)),onetype));                
            end
        elseif strcmp(opt.bin,'ITI')
            if opt.ctx_only
                typesel=find(meta_3.wrs_p(11,:)<0.05 & ismember(2-sign(meta_3.selec(11,:)),onetype) ...
                    & meta_6.wrs_p(14,:)<0.05 & ismember(2-sign(meta_6.selec(14,:)),onetype) & strcmp(meta.reg_tree(2,:),'CTX') & ~strcmp(meta.reg_tree(5,:),''));                
            else
                typesel=find(meta_3.wrs_p(11,:)<0.05 & ismember(2-sign(meta_3.selec(11,:)),onetype) & meta_6.wrs_p(14,:)<0.05 & ismember(2-sign(meta_6.selec(14,:)),onetype));                
            end
        else
            if opt.ctx_only
    ...
    elseif strcmp(type,'3s')
        if strcmp(opt.bin,'sample')
            if opt.ctx_only
                typesel=find(meta_3.wrs_p(4,:)<0.05 & ismember(2-sign(meta_3.selec(4,:)),onetype) & strcmp(meta.reg_tree(2,:),'CTX') & ~strcmp(meta.reg_tree(5,:),''));
            else
                typesel=find(meta_3.wrs_p(4,:)<0.05 & ismember(2-sign(meta_3.selec(4,:)),onetype));
            end
        elseif strcmp(opt.bin,'test')
            if opt.ctx_only
                typesel=find(meta_3.wrs_p(8,:)<0.05 & ismember(2-sign(meta_3.selec(8,:)),onetype) ...
                    & strcmp(meta.reg_tree(2,:),'CTX') & ~strcmp(meta.reg_tree(5,:),''));
            else
                typesel=find(meta_3.wrs_p(8,:)<0.05 & ismember(2-sign(meta_3.selec(8,:)),onetype));
            end
        elseif strcmp(opt.bin,'ITI')
    ...
    elseif strcmp(type,'6s')
        if strcmp(opt.bin,'sample')
            if opt.ctx_only
                typesel=find(meta_6.wrs_p(4,:)<0.05 & ismember(2-sign(meta_6.selec(4,:)),onetype) & strcmp(meta.reg_tree(2,:),'CTX') & ~strcmp(meta.reg_tree(5,:),''));
            else
                typesel=find(meta_6.wrs_p(4,:)<0.05 & ismember(2-sign(meta_6.selec(4,:)),onetype));
            end
        elseif strcmp(opt.bin,'test')
            if opt.ctx_only
                typesel=find(meta_3.wrs_p(8,:)<0.05 & ismember(2-sign(meta_3.selec(8,:)),onetype) ...
                ...
                typesel=find(meta_6.wrs_p(11,:)<0.05 & ismember(2-sign(meta_6.selec(11,:)),onetype));                
            end
        elseif strcmp(opt.bin,'ITI')
            if opt.ctx_only
                typesel=find(meta_6.wrs_p(11,:)<0.05 & ismember(2-sign(meta_6.selec(11,:)),onetype) ...
                ...
                typesel=find(meta_6.wrs_p(14,:)<0.05 & ismember(2-sign(meta_6.selec(14,:)),onetype));                
            end
        else
            if opt.ctx_only
                typesel=find(~ismember(meta_3.mem_type,onetype) & ismember(meta_6.mem_type,onetype) & strcmp(meta.reg_tree(2,:),'CTX') & ~strcmp(meta.reg_tree(5,:),''));
